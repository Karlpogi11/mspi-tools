      ...
      <div className="bg-white rounded-xl border border-[#d2d2d7] p-6">
        {disabled && (
          <div className="mb-4 px-4 py-3 rounded-lg bg-[#fef2f2] border border-[#fecaca] text-[13px] text-[#dc2626] flex items-center gap-2">
            ...
            Scanning is active on {activeScannerCount} device{activeScannerCount === 1 ? '' : 's'} — file replacement is locked.
          </div>
        )}
    ...
    return (
      <div>
        {disabled && (
          <div className="mb-4 px-4 py-3 rounded-lg bg-[#fef2f2] border border-[#fecaca] text-[13px] text-[#dc2626] flex items-center gap-2">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="10" /><line x1="12" y1="8" x2="12" y2="12" /><line x1="12" y1="16" x2="12.01" y2="16" /></svg>
            Import is locked — {activeScannerCount} device{activeScannerCount === 1 ? '' : 's'} are actively scanning. Wait until scanning finishes.
          </div>
        )}
        {!file ? (
          ...
          <div>
            ...
            {headers.length > 0 && (
              <>
                ...
                <div className="mb-4">
                  <p className="text-[12px] font-medium text-[#6e6e73] mb-1">Preview (first {preview.length} rows{mapping.displayColumns.length > 0 ? ` — showing ${mapping.displayColumns.length + 2} selected columns` : ''})</p>
