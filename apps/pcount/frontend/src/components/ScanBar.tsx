...
export default function ScanBar({ sessionId, onScanned, onScanQueued, onScanFailed }: Props) {
  ...
  return (
    <div className={`bg-white rounded-xl border shadow-lg p-4 transition-colors duration-200 ${
      overscan ? 'border-[#dc2626] ring-2 ring-[#dc2626]/30' : 'border-[#d2d2d7]'
    }`}>
      <form onSubmit={handleSubmit} className="flex gap-3">
        <div className="flex-1 relative">
          <div className="absolute left-3 top-1/2 -translate-y-1/2 text-[#6e6e73]">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 border border-[#d2d2d7] rounded-lg text-[16px] font-mono bg-[#f5f5f7] focus:outline-none focus:border-[#2563eb] focus:ring-1 focus:ring-[#2563eb] focus:bg-white transition-colors"
            autoComplete="off"
            autoFocus
          />
        </div>
        <button
          type="submit"
          disabled={!value.trim()}
          className="px-5 py-3 bg-[#2563eb] text-white text-[14px] font-medium rounded-lg hover:bg-[#1d4ed8] disabled:opacity-40 transition-colors cursor-pointer"
        >
