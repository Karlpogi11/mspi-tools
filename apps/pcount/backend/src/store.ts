import { getDb, getDbPool } from '@mspi/shared-db';
import { pcountSessions, pcountDisplayColumns, pcountProducts, pcountProductExtra } from '@mspi/shared-db/schema';
...
export async function scanProduct(sessionId: number, product_code: string): Promise<{ product: ProductWithExtra; match: boolean } | null> {
  if (dbAvailable) {
    ...
    // Keep the increment and status transition in one atomic statement. The
    // previous read-modify-write flow could lose a scan when two scanners hit
    // the same product at nearly the same time.
    await pool.execute(
