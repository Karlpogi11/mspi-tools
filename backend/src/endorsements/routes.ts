...
function canView(req: Request) { return canEndorse(req) || isEngineer(req); }
...
router.get('/available', async (req, res) => {
  ...
  await ensureFrontlineTables();
  const [rows] = await getDbPool().query(`SELECT a.id, a.user_id, a.engineer_name AS full_name, a.assignment_count, a.joined_at, a.last_assigned_at
    FROM engineer_daily_availability a LEFT JOIN users u ON u.id = a.user_id
    WHERE a.availability_date = ? AND a.status = 'active'
    ORDER BY a.assignment_count ASC, a.last_assigned_at IS NOT NULL ASC, a.last_assigned_at ASC, a.joined_at ASC, a.id ASC`, [todayManila()]);
  res.json({ date: todayManila(), engineers: rows, nextEngineer: (rows as Array<Record<string, unknown>>)[0] || null });
});

router.post('/', async (req, res) => {
  if (!canEndorse(req)) { forbidden(res); return; }
  const arNumber = text(req.body?.arNumber); const requestedAvailabilityId = Number(req.body?.availabilityId) || null; const recordId = Number(req.body?.frontlineRecordId) || null;
  ...
  try {
    ...
    const [engineerRows] = await connection.query(`SELECT a.id, a.user_id, a.engineer_name AS full_name, a.assignment_count FROM engineer_daily_availability a WHERE a.availability_date = ? AND a.status = 'active' ${requestedAvailabilityId ? 'AND a.id = ?' : ''} ORDER BY a.assignment_count ASC, a.last_assigned_at IS NOT NULL ASC, a.last_assigned_at ASC, a.joined_at ASC, a.id ASC FOR UPDATE`, requestedAvailabilityId ? [todayManila(), requestedAvailabilityId] : [todayManila()]);
    const engineer = (engineerRows as Array<Record<string, unknown>>)[0];
    if (!engineer) { await connection.rollback(); res.status(409).json({ error: requestedAvailabilityId ? 'That Engineer is not currently available.' : 'No Engineer is available today. Ask an Engineer to join the morning availability list.' }); return; }
    ...
    await connection.execute(`INSERT INTO engineer_endorsements (ar_number, frontline_record_id, cso_user_id, engineer_user_id, engineer_name, device_model, issue, product_division) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [arNumber, sourceRecordId, req.user!.userId, engineerId, engineerName, text(record.device_model), text(record.issue), text(record.product_division)]);
    ...
    res.status(201).json({ message: `AR ${arNumber} endorsed to ${engineerName}.`, engineer: { userId: engineerId, name: engineerName }, record: { arNumber, deviceModel: text(record.device_model), issue: text(record.issue), productDivision: text(record.product_division) } });
