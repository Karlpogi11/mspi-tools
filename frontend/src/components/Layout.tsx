...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen bg-[#F4F3F6]">
      ...
      <main className={isWideEndorsementsPage || isWideFrontlinePage ? 'mx-auto w-full max-w-none px-[clamp(1rem,3vw,3rem)] py-8 lg:py-10' : isWidePartsPage ? 'w-full max-w-none px-0 py-0' : 'mx-auto w-full max-w-[1200px] px-6 py-10'}>
        {isToolPage && !isFrontlineDataEntryPage && (
          <Link to={isPcountSessionPage ? '/pcount' : '/'} className={`print:hidden inline-flex items-center gap-1.5 mb-5 text-[12px] font-medium text-[#6e6e73] hover:text-[#2563eb] transition-colors ${isWidePartsPage ? 'ml-4 sm:ml-8 lg:ml-10 xl:ml-12' : ''}`}>
