  ...
  return (
    <div className="min-h-[calc(100vh-128px)] bg-[#f4f3f6]">
      ...
      <div className="overflow-hidden rounded-xl bg-white shadow-[0_1px_3px_#00000012]">
        <div className="grid grid-cols-[1.1fr_1fr_1fr_.8fr_.7fr_1.1fr] gap-3 bg-[#fafafd] px-4 py-2 text-[10px] font-semibold uppercase tracking-[.08em] text-[#86868b]"><span>Date</span><span>Site</span><span>ShipTo</span><span>Status</span><span>Total qty</span><span>Email</span></div>
        {filtered.length === 0 ? <div className="p-12 text-center text-[13px] text-[#6e6e73]">{connected ? 'No AppleCare packing lists found in the last 7 days.' : 'Connect Gmail to begin.'}</div> : filtered.map((row) => <button key={row.id} className="grid w-full cursor-pointer grid-cols-[1.1fr_1fr_1fr_.8fr_.7fr_1.1fr] gap-3 border-t border-[#f0f0f2] px-4 py-2 text-left text-[11px] leading-4 text-[#3c3c43] transition hover:bg-[#fafafd]" onClick={async () => setSelected(await api.applecare.detail(row.id))}><span>{formatDate(row.packing_date || row.received_at)}<small className="block text-[10px] text-[#86868b]">{formatTime(row.packing_time)}</small></span><span>{row.site_name || <span className="text-[#b0b0b5]">Unmapped</span>}</span><span className="font-medium">{row.ship_to}</span><span>{row.status}</span><span className="font-medium">{row.total_quantity}</span><span className="min-w-0"><a className="font-medium text-[#0071e3] hover:underline" href={row.email_url} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()}>Open email</a></span></button>)}
      </div>
