...
export default function EngineerEndorsementsPage() {
  ...
  const openCalendarEditor = (date: string, division: string, engineer: string) => { if (!canEditEndorsement) return; const entry = calendar?.days.find((day) => day.date === date)?.counts[division]?.[engineer]?.find((item) => item.is_manual); setEditingCell({ date, division, engineer, count: entry?.manual_count ? String(entry.manual_count) : '', details: entry?.details || '' }); };
  const saveCalendarEntry = async () => { if (!editingCell) return; const count = Number(editingCell.count); if (!Number.isInteger(count) || count < 1) return; setSavingCell(true); setMessage(''); try { await api.endorsements.saveCalendarEntry({ ...editingCell, count }); setEditingCell(null); setSelectedDay(null); await load(true); } catch (error) { showMessage((error as Error).message, true); } finally { setSavingCell(false); } };
  ...
  const canEditEndorsement = user?.roleName === 'Admin' || user?.roleName === 'ENGR';
