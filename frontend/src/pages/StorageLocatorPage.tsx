...
import { api, type StorageEmployee, type StorageMovement, type StorageRecentMovement, type StorageRule, type StorageUnit } from '../lib/api';
...
export default function StorageLocatorPage() {
  ...
  const [recentHistory, setRecentHistory] = useState<StorageRecentMovement[]>([]);
  ...
  const loadRecentHistory = async () => {
    const result = await api.storageLocator.recentHistory();
    setRecentHistory(result.history);
  };
  useEffect(() => { if (!employee) return; void Promise.all([loadOverview(), loadRecentHistory()]).catch((err) => setError((err as Error).message)); }, [employee]);
  ...
  const lookup = async () => {
    ...
    try { const result = await api.storageLocator.lookup(arNumber.trim()); setUnit(result.unit); setHistory(result.history); setShowFullHistory(false); setAction(result.unit?.state === 'in' ? 'OUT' : 'IN'); }
  ...
  const submitIn = async () => {
    ...
    setBusy(true); setError('');
    try { const result = await api.storageLocator.checkIn({ employeeNumber: employee.employeeNumber, arNumber: arNumber.trim(), family, status, cabinetNumber: cabinet }); setMessage(result.message); await Promise.all([lookup(), loadOverview(), loadRecentHistory()]); }
    catch (err) { setError((err as Error).message); }
    finally { setBusy(false); }
  };
  const submitOut = async () => {
    if (!employee || !arNumber.trim()) return;
    setBusy(true); setError('');
    try { const result = await api.storageLocator.checkOut({ employeeNumber: employee.employeeNumber, arNumber: arNumber.trim() }); setMessage(result.message); await Promise.all([lookup(), loadOverview(), loadRecentHistory()]); }
    catch (err) { setError((err as Error).message); }
    finally { setBusy(false); }
  };
  const hasArSearch = Boolean(arNumber.trim());
  const visibleHistory = showFullHistory ? history : history.slice(0, 10);
  ...
  return <div className="min-h-[calc(100vh-128px)] bg-[#f4f3f6]">
    <div className="mx-auto max-w-6xl">
      ...
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1.35fr)_minmax(300px,.65fr)]">
        <section className="rounded-2xl border border-[#e5e5e7] bg-white p-4 shadow-[0_8px_28px_rgba(0,0,0,.03)]">
          <div className="flex items-start justify-between gap-4"><div><h2 className="text-[17px] font-semibold tracking-tight text-[#1d1d1f]">Storage Locator</h2><p className="mt-1 text-[13px] leading-5 text-[#6e6e73]">Find a customer unit, see its history, and place it in the right cabinet.</p></div><span className={`shrink-0 rounded-full px-2.5 py-1 text-[10px] font-semibold ${employee ? 'bg-[#ecfdf3] text-[#166534]' : 'bg-[#f5f5f7] text-[#6e6e73]'}`}>{employee ? 'Verified' : 'Not verified'}</span></div>
          ...
          <div className="mt-5 rounded-2xl bg-[#f7f7f8] p-4"><label className="text-[11px] font-semibold text-[#3c3c43]">Search by AR number<input autoFocus={Boolean(employee)} value={arNumber} onChange={(e) => { setArNumber(e.target.value); setUnit(null); setHistory([]); setShowFullHistory(false); setAction(null); setCabinet(null); }} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); void lookup(); } }} disabled={!employee} placeholder={employee ? 'Enter AR number' : 'Verify employee first'} className="mt-1 h-12 w-full rounded-xl border border-[#c7c7cc] bg-white px-3 text-[15px] outline-none transition-shadow placeholder:text-[#9a9aa1] focus:border-[#1d1d1f] focus:ring-2 focus:ring-[#1d1d1f]/10 disabled:bg-[#f5f5f7]" /></label><p className="mt-2 text-[11px] text-[#6e6e73]">Leave this blank to see the latest storage activity.</p><button type="button" onClick={() => void lookup()} disabled={busy || !employee || !arNumber.trim()} className="mt-3 w-full rounded-2xl bg-[#1d1d1f] py-2.5 text-[12px] font-semibold text-white transition-colors hover:bg-[#333336] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1d1d1f]/20 disabled:opacity-40">{busy ? 'Looking up…' : 'View unit history'}</button></div>
        ...
        <section className="h-full min-h-full max-h-none overflow-y-auto rounded-2xl border border-[#e5e5e7] bg-white p-4"><div className="flex items-center justify-between gap-3"><div><h2 className="text-[15px] font-semibold text-[#1d1d1f]">{hasArSearch ? 'Unit history' : 'Recent History'}</h2><p className="mt-1 text-[12px] text-[#6e6e73]">{hasArSearch ? (history.length ? `${Math.min(history.length, 10)}${hasMoreHistory ? ` of ${history.length}` : ''} recorded operation${history.length === 1 ? '' : 's'}` : 'No history found for this AR number.') : (recentHistory.length ? 'Latest storage activity' : 'No storage activity recorded yet.')}</p></div><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#f5f5f7] text-[#6e6e73]">↕</div></div>{!hasArSearch ? <div className="mt-5 space-y-4">{recentHistory.length ? recentHistory.map((entry, index) => <div key={entry.id} className="relative flex gap-3"><div className="flex w-5 flex-col items-center"><span className={`mt-1 h-2.5 w-2.5 rounded-full ${entry.action === 'IN' ? 'bg-[#34a853]' : 'bg-[#9ca3af]'}`} />{index < recentHistory.length - 1 && <span className="mt-1 h-full w-px bg-[#e5e5e7]" />}</div><div className="min-w-0 flex-1 pb-1"><div className="flex items-center justify-between gap-2"><span className="text-[12px] font-semibold text-[#1d1d1f]">{entry.action} · {entry.family} {entry.cabinet_number ? pad(entry.cabinet_number) : '—'}</span><span className="whitespace-nowrap text-[10px] text-[#86868b]">{dateTime(entry.occurred_at)}</span></div><p className="mt-1 text-[11px] text-[#6e6e73]">AR {entry.ar_number} · {entry.status} · {entry.full_name} ({entry.employee_number})</p></div></div>) : <div className="rounded-xl border border-dashed border-[#d2d2d7] p-6 text-center text-[12px] text-[#6e6e73]">No recent history loaded yet.</div>}</div> : <div className="mt-5 space-y-4">{visibleHistory.length ? visibleHistory.map((entry, index) => <div key={entry.id} className={`relative flex gap-3 ${hasMoreHistory && index === visibleHistory.length - 1 ? 'after:pointer-events-none after:absolute after:inset-x-0 after:bottom-0 after:h-10 after:bg-gradient-to-b after:from-transparent after:to-white' : ''}`}><div className="flex w-5 flex-col items-center"><span className={`mt-1 h-2.5 w-2.5 rounded-full ${entry.action === 'IN' ? 'bg-[#34a853]' : 'bg-[#9ca3af]'}`} />{index < visibleHistory.length - 1 && <span className="mt-1 h-full w-px bg-[#e5e5e7]" />}</div><div className="min-w-0 flex-1 pb-1"><div className="flex items-center justify-between gap-2"><span className="text-[12px] font-semibold text-[#1d1d1f]">{entry.action} · {entry.family} {entry.cabinet_number ? pad(entry.cabinet_number) : '—'}</span><span className="whitespace-nowrap text-[10px] text-[#86868b]">{dateTime(entry.occurred_at)}</span></div><p className="mt-1 text-[11px] text-[#6e6e73]">{entry.status} · {entry.full_name} ({entry.employee_number})</p></div></div>) : <div className="rounded-xl border border-dashed border-[#d2d2d7] p-6 text-center text-[12px] text-[#6e6e73]">No history found for this AR number.</div>}{history.length > 10 && <button type="button" onClick={() => setShowFullHistory((current) => !current)} className="mx-auto mt-1 block w-fit text-center text-[12px] font-semibold text-[#3c3c43] hover:text-[#1d1d1f] hover:underline">{showFullHistory ? 'Show less' : 'View more'}</button>}</div>}</section>
      </div>
    </div>
