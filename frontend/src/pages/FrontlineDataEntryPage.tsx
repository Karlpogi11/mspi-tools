...
export default function FrontlineDataEntryPage() {
  ...
  return <div className="min-h-[calc(100vh-128px)] bg-[#f5f5f7] px-5 py-8 sm:px-8 lg:py-12">
    <div className="mx-auto w-full max-w-[820px]">
      <nav aria-label="Breadcrumb" className="mb-12 flex items-center gap-2 text-[12px] text-[#6e6e73]"><Link to="/" className="hover:text-[#1d1d1f]">Tools</Link><Chevron /><Link to="/frontline" className="hover:text-[#1d1d1f]">Frontline Monitor</Link><Chevron /><span className="text-[#1d1d1f]">Data Entry</span></nav>
      <section className="overflow-hidden rounded-lg border border-[#e5e5e5] bg-white">
        <header className="border-b border-[#eeeeee] px-7 pb-8 pt-8 sm:px-9 sm:pb-9 sm:pt-9"><h1 className="text-[28px] font-normal tracking-[-0.02em] text-[#1d1d1f]">Frontline Data Entry</h1><p className="mt-3 max-w-[620px] text-[13px] leading-5 text-[#6e6e73]">Enter a new record using the exact columns from the website-entry worksheet. Source worksheets remain read-only.</p></header>
        <form onSubmit={(event) => { event.preventDefault(); void submit(); }} className="px-7 py-8 sm:px-9 sm:py-9">
          {message && <div role="status" aria-live="polite" className="mb-8 rounded-lg bg-[#f5f5f7] px-4 py-3 text-[13px] text-[#3c3c43]">{message}</div>}
          <div className="mb-9 grid grid-cols-1 gap-2 border-b border-[#f0f0f0] pb-8 sm:grid-cols-[170px_minmax(0,360px)] sm:items-start sm:gap-x-8"><label className="pt-2 text-[13px] text-[#3c3c43]">Destination worksheet</label><div><input value={loading ? 'Loading…' : sheet || 'Not configured by Admin'} readOnly className="h-9 w-full rounded-lg border border-[#e8e8e8] bg-[#fbfbfb] px-3 text-[13px] text-[#3c3c43] outline-none" /><p className="mt-2 text-[11px] leading-4 text-[#86868b]">New entries append only to this worksheet.</p></div></div>
...
function SmartField({ header, value, onChange }: { header: string; value: string; onChange: (value: string) => void }) {
  ...
  return <label className="block min-w-0 text-[13px] text-[#3c3c43]"><span className="block min-h-[18px]">{header}</span>{multiline ? <textarea rows={4} value={value} placeholder={placeholder} aria-label={header} onChange={(event) => onChange(event.target.value)} className="mt-2 block w-full resize-y rounded-lg border border-[#e8e8e8] px-3 py-2 text-[13px] leading-5 text-[#1d1d1f] outline-none placeholder:text-[#a1a1a6] focus:border-[#0071c8] focus:ring-2 focus:ring-[#0071c8]/10" /> : <input type={type} value={value} placeholder={placeholder} aria-label={header} autoComplete="off" enterKeyHint="next" onKeyDown={isTime ? setCurrentTime : undefined} onChange={(event) => onChange(event.target.value)} className="mt-2 block h-9 w-full rounded-lg border border-[#e8e8e8] px-3 text-[13px] text-[#1d1d1f] outline-none placeholder:text-[#a1a1a6] focus:border-[#0071c8] focus:ring-2 focus:ring-[#0071c8]/10" />}{isTime && <span className="mt-1.5 block text-[11px] text-[#86868b]">Press ⌘ ⇧ ; for the current time</span>}</label>;
}
