...
export default function EngineerEndorsementsBoardPage() {
  ...
  const progress = useMemo(() => endorsedRows.filter((row) => !['cancelled', 'canceled', 'completed', 'resolved'].includes(row.status.toLowerCase()) && (!viewEngineer || row.engineer_name === viewEngineer)), [endorsedRows, viewEngineer]);
  ...
  const renderCard = (record: WaitingRecord | BoardRecord, isNew: boolean) => {
    ...
    return <button key={record.id} type="button" onClick={() => setSelected(record)} className={`w-full rounded-[18px] border p-4 text-left shadow-[0_1px_3px_rgba(0,0,0,.04)] transition hover:-translate-y-0.5 hover:shadow-[0_6px_18px_rgba(0,0,0,.08)] ${isNew ? 'border-[#b9cee5] bg-[#f8fbff]' : 'border-[#d9d9de] bg-white'}`}>
      <div className="flex items-start justify-between gap-4"><div className="min-w-0"><p className={`text-[11px] font-semibold normal-case tracking-normal ${isNew ? 'text-[#35658f]' : 'text-[#86868b]'}`}>{isNew ? 'New Frontline Endorsement' : 'Endorsement'}</p><h3 className="mt-1 text-[18px] font-semibold tracking-[-.02em] text-[#1d1d1f]">{record.ar_number}</h3></div><span className={`max-w-[48%] shrink-0 truncate rounded-full px-2.5 py-1.5 text-[11px] font-semibold ${assigned ? 'bg-[#f1f1f3] text-[#3c3c43]' : isNew ? 'bg-[#e8f1fb] text-[#35658f]' : 'bg-[#f1f1f3] text-[#5f5f66]'}`}>{assigned || 'Choose Engineer'}</span></div>
      <div className="my-4 border-t border-[#ececf0]" /><div className="grid gap-3"><div><p className="text-[10px] font-medium uppercase tracking-[.12em] text-[#86868b]">Device</p><p className="mt-1 text-[13px] font-medium text-[#3c3c43]">{record.device_model || ''}</p></div><div><p className="text-[10px] font-medium uppercase tracking-[.12em] text-[#86868b]">Issue</p><p className="mt-1 line-clamp-3 text-[13px] leading-5 text-[#4b4b52]">{record.issue || ''}</p></div></div>
      <div className="mt-4 flex flex-wrap items-center gap-x-2 gap-y-1 border-t border-[#ececf0] pt-3 text-[10px] text-[#86868b]"><span>{isNew ? (('cso' in record ? record.cso || 'Unknown' : 'Unknown') + ' · ' + record.product_division) : 'Last endorsed Engineer: ' + (assigned || 'Unassigned')}</span><span>·</span><span>{new Date(sourceDate).toLocaleString('en-PH', { timeZone: 'Asia/Manila' })}</span></div>
    </button>;
  };
